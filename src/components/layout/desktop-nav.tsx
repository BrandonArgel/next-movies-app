    ...
    <ul className="hidden xl:flex items-center gap-1 flex-1 ml-4" role="list">
      {menus.map((menu) => (
        <li key={menu.label}>
          <DropdownMenuTrigger>
            {/* 
              Usamos tu variante "ghost" o la que prefieras. 
              El data-[expanded] viene nativo de React Aria para saber si el menú está abierto 
            */}
            <Button
              variant="ghost"
            ...
            <DropdownMenu className="min-w-45">
              {menu.items.map((item) => (
                <DropdownMenuItem
                  key={item.href}
                  href={item.href} // React Aria maneja la navegación si le pasas href
